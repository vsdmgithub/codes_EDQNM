module parameters_mod
    ...
    integer (kind=4)::i
        ...
        ! Defining Viscosity
        vcs=4.4D0*(10**(-4.0))
        ! Defining constant in eddy viscosity
        a=0.54D0
        ...
        sc_vcs_time=1.0D0/(vcs*(k(N)**(2.0))) ! Time scale of smallest eddy
...
module function_mod
    ...
    cosine=(k(i1)**(2.0D0)+k(i2)**(2.0D0)-k(i3)**(2.0D0))/(2.0D0*k(i1)*k(i2))
    ...
    b=(k(i2)/k(i1))*(z**(3.0D0)-x*y) ! conventionally x has to be -x here, but our notation change has another negative sign
    ...
    m=m1+m2+m3
    w=vcs*(k(i1)**(2.0D0)+k(i2)**(2.0D0)+k(i3)**(2.0D0))
    th=(1.0D0-exp(-(m+w)*t))/(m+w)
    ...
    ef=((k(i1)**(2.0D0))*en(i2)-(k(i2)**(2.0D0))*en(i1))*en(i3)*k(i1)/(k(i2)*k(i3))
...
!    end do
    ...
    en_thres=2.0D0**(-60.0)
    do i=1,N
       en_init(i)=(k(i)**(2.0D0))*exp(-(k(i)**(2.0D0))/(1.0D0))+en_thres
      en_total=en_total+en_init(i)
    end do
    en0=1.0D0/(2.0D0*en_total)
    do i=1,N
       en_init(i)=en0*(k(i)**(2.0D0))*exp(-(k(i)**(2.0D0))/(1.0D0))+en_thres
    end do
...
!    close(10)
    ...
    subroutine print_parameters
     ...
     eddy_timeN=1.0D0/m_N
     eddy_time1=1.0D0/m_1
    ...
    do s=1,s_max-1
        ...
        en_temp=en_soln(:,s)+0.5D0*den1
        call EDQNM_eqn(en_temp,s,den2)
        en_temp=en_soln(:,s)+0.5D0*den2
        call EDQNM_eqn(en_temp,s,den3)
        en_temp=en_soln(:,s)+den3
        call EDQNM_eqn(en_temp,s,den4)
        en_soln(:,s+1)=en_soln(:,s)+(den1+2.0D0*(den2+den3)+den4)/6.0D0
        ! CHECK for "NaN"
    ...
    do i=1,N                                                                                                                                                            
        ...
        den(i)=dt*(tr_int2-2.0D0*vcs*(k(i)**(2.0D0))*en(i))
    end do
