   ...
   ! _________________________
    ...
    ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        ! Final increment for 'v(k)'
				...
				CALL thermal_noise
				! Gets a noise array for given temperature

				spec   =   spec + dt * th_noise * laplacian_k
...
!        flux_neg    =   flux_pos    -   flux
    ...
    END
        ...
        IMPLICIT NONE
				...
				CALL RANDOM_NUMBER(th_noise)

				th_noise = bath_temp * DSQRT( - two * DLOG( dummy ) ) * DCOS( two_pi * th_noise )

				th_noise = hf * ( DABS( th_noise ) + th_noise )
				! print*,th_noise
    END
